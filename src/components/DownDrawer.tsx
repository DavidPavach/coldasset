...
const DownDrawer = ({ isOpen, onClose, children, height = "80vh" }: DownDrawerProps) => {
        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    {/* Drawer */}
                    <motion.div className="right-0 bottom-0 left-0 z-50 fixed bg-background shadow-xlrounded-t-2xl" style={{ height }} variants={drawerVariants} initial="hidden" animate="visible"
                        exit="exit" transition={{ type: "spring", stiffness: 260, damping: 30 }} drag="y" dragConstraints={{ top: 0 }}
                        dragElastic={0.2}
                        onDragEnd={(_, info) => {
