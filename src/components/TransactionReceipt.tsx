...
// Icons
import { X, Share2, CircleCheckBig, XCircle, ArrowUpRight, ArrowDownLeft, ArrowRightLeft, CheckCheck } from 'lucide-react';
import { DocumentDownload, Clock, Copy } from 'iconsax-reactjs';
...
FROM: ${transaction.fromAmount} ${transaction.fromCoin?.toUpperCase()}
TO: ${transaction.toAmount} ${transaction.toCoin?.toUpperCase()}
` : `
...
Generated: ${formatDate(new Date())}
    ...
    return (
        <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="z-50 fixed inset-0 flex justify-center items-center bg-black/50 backdrop-blur-sm p-2" onClick={onClose}>
                <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
                    transition={{ type: "spring", duration: 0.5 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-lg max-h-[85vh] xl:max-h-[90vh] overflow-y-auto">
                    <Card className="shadow-2xl py-4 border-border">
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="bg-background py-4 rounded-2xl text-center">
                                ...
                                {transaction.transactionType === 'swap' ? (
                                    <div className="space-y-2">
                                        <div className="flex justify-center items-center gap-3">
                                            <div className="text-right">
                                                <p className="font-bold text-foreground text-xl md:text-2xl xl:text-3xl montserrat">
                                                    {transaction.fromAmount}
                                                </p>
                                                <p className="text-[11px] text-muted-foreground md:text-xs xl:text-sm">
                                                    {transaction.fromCoin?.toUpperCase()}
                                                </p>
                                            </div>
                                            ...
                                            <div className="text-left">
                                                ...
                                                <p className="text-[11px] text-muted-foreground md:text-xs xl:text-sm">
                                                    {transaction.toCoin?.toUpperCase()}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <h3 className={cn("mb-1 font-bold md:text-xl text-2xl xl:text-4xl montserrat", transaction.transactionType === 'sent' ? "text-red-600" : "text-green-600")}>
                                            {typeConfig.sign}{transaction.amount}
                                        </h3>
                                        <div className="flex justify-center items-center gap-2 text-muted-foreground">
                                            <img src={meta.logo} alt={meta.name + " logo"} className='size-7 md:size-8 xl:size-9' />
                            ...
                            <div className="space-y-2">
                                ...
                                {transaction.receiver && (
                                    <DetailRow label="Receiver" value={transaction.receiver} copyable
                                        onCopy={() => handleCopy(transaction.receiver ?? "", 'receiver')} copied={copied === 'receiver'} />
                                )}
    ...
    return (
        <div className="group flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                <span className="font-medium text-[11px] text-foreground md:text-xs xl:text-sm text-right capitalize">
                    {value}
                </span>
                {copyable && (
                    <Button variant="ghost" size="icon" className="size-7" onClick={() => onCopy?.(fullValue || value)}>
                        {copied ? (
