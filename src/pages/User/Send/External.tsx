...
import { formatCurrency, validateWallet } from "@/utils/format";
...
const External = ({ coin }: { coin: string }) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!validateWallet(formData.walletAddress, coin)) {
            errors.walletAddress = `Please enter a valid ${coin.toUpperCase()} address`;
        }
        ...
        <>
            ...
            <main className="mx-auto mt-10 max-w-6xl">
                <div className="mb-8 text-center">
                    ...
                    <p className="mt-2 text-muted-foreground">Cold Asset External Transfer</p>
                </div>
                ...
                <Card>
                    ...
                    <CardContent className="flex flex-col gap-y-5 pt-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input placeholder="Enter wallet address (e.g., 0x742d35Cc6634C0532925a3b844Bc9e7595f...)" value={formData.walletAddress}
                                onChange={(e) => handleInputChange("walletAddress", e.target.value)} className={`font-mono ${validationErrors.walletAddress ? "border-destructive" : ""}`} />
                            <p className="text-[10px] text-yellow-600 md:text-[11px] dark:text-yellow-500 xl:text-xs"> Double-check the wallet before submitting. Transfers cannot be reversed. </p>
                            ...
                            <div className="space-y-2 bg-muted mt-4 p-4 border border-border/50 rounded-lg">
                                ...
                                <div className="flex justify-between mt-2 pt-2 border-border/50 border-t text-xs">
                                    ...
                                    <span className="max-w-xs font-mono truncate">
                                        {formData.walletAddress || <Loader className="size-4 text-primary animate-spin" />}
                                    </span>
                                </div>
                                <div className="flex justify-between text-xs">
