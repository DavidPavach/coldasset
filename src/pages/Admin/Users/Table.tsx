...
// Icons
import { Ban, BadgeCheck, CircleCheckBig, Loader2, UserRoundCheck, UserRoundX } from 'lucide-react';

const Table = ({ users }: { users: AdminAllUsers[] }) => {
    ...
    const handleKycVerify = (email: string) => {
        handleUserUpdate(
            { email, kyc: { status: "accepted" } },
                        ...
                        {users.map(user => (
                            <tr key={user._id} className="*:px-4 *:py-3 border-border border-b">
                                ...
                                <td className="space-x-3">
                                    ...
                                    <button title="Verify KYC" onClick={() => handleKycVerify(user.email)} className={`bg-blue-500 hover:bg-blue-600 px-2 py-1 rounded text-white transition-colors cursor-pointer`}>
                                        {loadingEmail === user.email ? <Loader2 className="size-4 animate-spin" /> : <CircleCheckBig className="size-4" />}
                                    </button>
                                </td>
                            </tr>
                        ))}
